...
SELECT*
FROM EmployeeDemographics
GROUP BY Gender, EmployeeID, FirstName, LastName,Age
...
FROM Customer
INNER JOIN Sales1
	ON Customer.CustomerID =Sales1.CustomerID
INNER JOIN BONUS
	ON Sales1.CustomerID = BONUS.CustomerID

SELECT Customer.CustomerID,Sales1.SalesID, Gender, Goods,CostOfGoods,NumberOfGoods
FROM Customer
INNER JOIN Sales1
	ON Customer.CustomerID =Sales1.CustomerID
INNER JOIN BONUS
	ON Sales1.SalesID = BONUS.SalesID
...
SELECT FirstName, LastName, Age, Gender, COUNT(Gender), 
CASE
	WHEN Age BETWEEN 25 AND 26 THEN 'YOUNG'
	WHEN Age = 27 THEN 'Mature'
	WHEN Age BETWEEN 28 AND 29 THEN 'Grown'
...
WITH CTE_Employee AS (SELECT FirstName, LastName, Gender, Salary, Age,
	COUNT(Age) OVER (PARTITION BY Gender) AS TotalGender, AVG(Salary) OVER (PARTITION BY Gender) AS AvgSalary
FROM  EmployeeDemographics EMP
JOIN EmployeeSalary SAL
ON EMP.EmployeeID = SAL.EmployeeID
WHERE Salary > 30000)
...
SELECT FirstName, LastName, Age, Gender, COUNT(Age) OVER(PARTITION BY Age ORDER BY Gender) AS GenderCount
FROM EmployeeDemographics
